...
class MIP_Model(object):
    def __init__(
        ...
        self.edges = wedges + gedges
        self.check_graph(n_vertices, self.edges)
        self.n_vertices = n_vertices
        self.gedges = gedges
        self.wedges = wedges
        self.n_edges = len(gedges+wedges)
    ...
    def _add_variables(self):
        ...
        for i in range(self.num_subcircuit):
            subcircuit_x = []
            for j in range(len(self.gedges)):
                v = self.model.addVar(lb=0.0, ub=1.0, vtype=gp.GRB.BINARY)
                subcircuit_x.append(v)
            self.gedge_var.append(subcircuit_x)
        
        self.wedge_var = []
        for i in range(self.num_subcircuit):
            subcircuit_x = []
            for j in range(len(self.wedges)):
                v = self.model.addVar(lb=0.0, ub=1.0, vtype=gp.GRB.BINARY)
                subcircuit_x.append(v)
            self.wedge_var.append(subcircuit_x)

        """
        Total number of cuts
        add 0.1 for numerical stability
        """
        self.num_cuts_g = self.model.addVar(
            lb=0, ub=self.max_cuts + 0.1, vtype=gp.GRB.INTEGER, name="num_cuts_gate"
        )
        self.num_cuts_w = self.model.addVar(
            lb=0, ub=self.max_cuts + 0.1, vtype=gp.GRB.INTEGER, name="num_cuts_wire"
        )
        ...
        for subcircuit in range(self.num_subcircuit):
            ...
            self.subcircuit_counter[subcircuit]["init"] = self.model.addVar(
                lb=0,
                ub=self.max_subcircuit_width,
                vtype=gp.GRB.INTEGER,
                name="init_%d" % subcircuit,
            )
            self.subcircuit_counter[subcircuit]["meas"] = self.model.addVar(
                lb=0,
                ub=self.max_subcircuit_width,
                vtype=gp.GRB.INTEGER,
                name="meas_%d" % subcircuit,
            )
            self.subcircuit_counter[subcircuit]["Q"] = self.model.addVar(
                lb=0.1,
                ub=self.max_subcircuit_width,
                vtype=gp.GRB.INTEGER,
                name="Q_%d" % subcircuit,
            )
    ...
    def _add_constraints(self):
        ...
        """
        for i in range(self.num_subcircuit):
            for e in range(len(self.gedges)):
                u, v = self.gedges[e]
                u_vertex_var = self.vertex_var[i][u]
                v_vertex_var = self.vertex_var[i][v]
                self.model.addConstr(self.gedge_var[i][e] <= u_vertex_var + v_vertex_var)
                self.model.addConstr(self.gedge_var[i][e] >= u_vertex_var - v_vertex_var)
                self.model.addConstr(self.gedge_var[i][e] >= v_vertex_var - u_vertex_var)
                self.model.addConstr(
                    self.gedge_var[i][e] <= 2 - u_vertex_var - v_vertex_var
                )
            for e in range(len(self.wedges)):
                u, v = self.wedges[e]
                u_vertex_var = self.vertex_var[i][u]
                v_vertex_var = self.vertex_var[i][v]
                self.model.addConstr(self.wedge_var[i][e] <= u_vertex_var + v_vertex_var)
                self.model.addConstr(self.wedge_var[i][e] >= u_vertex_var - v_vertex_var)
                self.model.addConstr(self.wedge_var[i][e] >= v_vertex_var - u_vertex_var)
                self.model.addConstr(
                    self.wedge_var[i][e] <= 2 - u_vertex_var - v_vertex_var
                )
        """
        ...
        for subcircuit in range(self.num_subcircuit):
            ...
            self.model.addConstr(
                self.subcircuit_counter[subcircuit]["init"]
                == gp.quicksum(
                    [
                        self.gedge_var[subcircuit][i]
                    ...
                    [
                        self.wedge_var[subcircuit][i]
                        * self.vertex_var[subcircuit][self.edges[i+len(self.gedges)][1]]
                        for i in range(len(self.wedges))
                    ]
                )
            )
            ...
            self.model.addConstr(
                self.subcircuit_counter[subcircuit]["meas"]
                == gp.quicksum(
                    [
                        self.gedge_var[subcircuit][i]
                    ...
                    [
                        self.wedge_var[subcircuit][i]
                        * self.vertex_var[subcircuit][self.edges[i+len(self.gedges)][0]]
                        for i in range(len(self.wedges))
                    ]
                )
            )
            ...
            self.model.addConstr(
                self.subcircuit_counter[subcircuit]["Q"]
                == self.subcircuit_counter[subcircuit]["original_input"]
                + self.subcircuit_counter[subcircuit]["init"]
            )
        ...
        self.model.setObjective(self.num_cuts_g+self.num_cuts_w, gp.GRB.MINIMIZE)
        self.model.update()
    ...
    def solve(self):
        ...
        if self.model.solcount > 0:
            ...
            for i in range(self.num_subcircuit):
                for j in range(len(self.gedges)):
                    if abs(self.gedge_var[i][j].x) > 1e-4 and j not in cut_edges_idx:
                        cut_edges_idx.append(j)
                        u, v = self.gedges[j]
                        cut_edges.append((self.id_vertices[u], self.id_vertices[v]))
                        self.cnot_cut.append((self.id_vertices[u], self.id_vertices[v]))
                for j in range(len(self.wedges)):
                    if abs(self.wedge_var[i][j].x) > 1e-4 and j+len(self.gedges) not in cut_edges_idx:
                        cut_edges_idx.append(j+len(self.gedges))
                        u, v = self.wedges[j]
                        cut_edges.append((self.id_vertices[u], self.id_vertices[v]))
...
def get_counter(subcircuits, O_rho_pairs):
    ...
    for pair in O_rho_pairs:
        ...
        counter[O_qubit["subcircuit_idx"]]["meas"] += 1
        counter[rho_qubit["subcircuit_idx"]]["init"] += 1
    return counter

def translate_cnot(circuit, location):
    ...
    for ct in location:
        ...
        circuit.data.insert(counter,(op_c,[Qubit(QuantumRegister(circuit.num_qubits, 'q'), qb[0])],[]))
        circuit.data.insert(counter+1,(op_t,[Qubit(QuantumRegister(circuit.num_qubits, 'q'), qb[1])],[]))
        seq_cnot += 1
    return circuit
...
def find_cuts(
    ...
    num_cnot = len(circuit_to_dag(circuit).collect_runs(['cx']))
    ...
    for num_subcircuit in num_subcircuits:
        ...
        feasible = mip_model.solve()
        if feasible:
            # print(mip_model.subcircuits)
            # print(len(mip_model.vertex_var))
            print([[j.x for j in mip_model.vertex_var[i]] for i in range(len(mip_model.vertex_var))])
            positions = cuts_parser(mip_model.cut_edges, circuit, vertex_ids)
            # print(mip_model.cut_edges)
            subcircuits, complete_path_map = subcircuits_parser(
                subcircuit_gates=mip_model.subcircuits, circuit=translate_cnot(circuit,mip_model.cnot_cut)
            )
            # print(mip_model.cnot_cut)
...
def print_cutter_result(num_cuts, subcircuits, counter):
    ...
    for subcircuit_idx in range(len(subcircuits)):
        ...
        print(
            "init qubits = %d, measure qubits = %d, width = %d, effective = %d, depth = %d, size = %d"
            % (
                counter[subcircuit_idx]["init"],
                counter[subcircuit_idx]["meas"],
                counter[subcircuit_idx]["Q"],
        ...
        print('Resource needed:',  PhysicalParameters.make_beverland_et_al(subcircuits[subcircuit_idx]))
