...
def estimate_resources(circuit_element: Any, Toffoli: bool = False) -> Dict:
    ...
    """
    try:
        resource_dict = {'LogicalQubits':circuit_element.num_qubits}
    except:
        resource_dict={}
    dag_dict = circuit_to_dag(circuit_element)._op_names
    if 't' in dag_dict:
        resource_dict["T"] = dag_dict['t'] 
    else:
        resource_dict["T"] = 0
    if 'cx' in dag_dict: 
        resource_dict["cnot"] = dag_dict['cx']
    else:
        resource_dict["cnot"] = 0

    resource_dict["Rotations"] = 0
    if 'rx' in dag_dict:
        resource_dict["Rotations"] = dag_dict['rx'] + resource_dict.get("Rotations")
    if 'ry' in dag_dict:
        resource_dict["Rotations"] = dag_dict['ry'] + resource_dict.get("Rotations")
    if 'rz' in dag_dict:
        resource_dict["Rotations"] = dag_dict['rz'] + resource_dict.get("Rotations")

    if not Toffoli:
        resource_dict["Toffoli"] = 0
    else:
        resource_dict["Toffoli"] = dag_dict['ccx']
    resource_dict["Rotation_layer"] = np.ceil(resource_dict.get("Rotations")/circuit_element.num_qubits)
    # print(resource_dict)
    return resource_dict
